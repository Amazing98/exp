...
class SingleSwitchTopo(Topo):
    ...
    def build(self, loss=0, bw_client=0, bw_inner=0, delayMs=0):
        delay = str(delayMs) + 'ms'
        switch = self.addSwitch('s1')
        # Each host gets 50%/n of systEM CPU

        host1 = self.addHost('h1', cpu=.5 / 2)
        host2 = self.addHost('h2', cpu=.5 / 2)
        host3 = self.addHost('h3', cpu=.5 / 2)

        # host1 uses bw client
        self.addLink(host1, switch,
                     bw=bw_client, delay=delay, loss=loss, use_htb=True)

        # host2 uses bw server
        self.addLink(host2, switch,
                     bw=bw_inner, delay=delay, loss=loss, use_htb=True)
...
def perfTest():
    ...
    topo = SingleSwitchTopo(loss=0, bw_client=1, bw_inner=100, delayMs=0)
    info("current loss is %d , bw_client is %d mbps, bw_inner is %d mbps\n" % (0, 1, 100))
